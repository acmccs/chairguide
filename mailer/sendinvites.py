...
def send_invite(name, email, fromaddr):
    toaddr = email
    replytoaddr = CHAIRS_ADDR
    cc = [CHAIRS_ADDR]
    message_subject = "ACM CCS 2017 PC Invitation"
    with open ("invitation.txt", "r") as myfile:
...
if __name__=="__main__":
    ...
    for p in pc:
        print(p["Name"] + " - " + p["Email"] + " / " + p["Contact"])
        assert '@' in p["Email"] 
        assert p["Contact"] in fromaddrs
    # fromaddr = 'evans@virginia.edu'
    # fromaddr = 'tal@columbia.edu'


    for p in pc:
        send_invite(p["Name"], p["Email"], fromaddrs[p["Contact"]])
