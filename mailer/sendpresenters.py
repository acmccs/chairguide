...
def send_invite(name, email, fromaddr):
    ...
    message_subject = "ACM CCS 2017 Presenter Information"
    with open ("presenter.txt", "r") as myfile:
...
if __name__=="__main__":
    pc = read_pc("presenters.csv")
    fromaddrs = {'Dave': 'David Evans <evans@virginia.edu>' }

    print ("Read PC list: " + str(len(pc)))
    for p in pc:
        print(p["Presenter"] + " - " + p["PresenterEmail"])
        assert '@' in p["PresenterEmail"] 

    for p in pc:
        send_invite(p["Presenter"], p["PresenterEmail"], fromaddrs['Dave'])
